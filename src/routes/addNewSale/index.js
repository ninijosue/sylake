...
export default class AddNewSale extends Component {
    ...
    async getProducts(siteName) {
        ...
        if (productsDefine == "custom") {
            const products = await ProductModel.getProductsWhenDefinedAsCustom(primaryId, siteName);
            this.setState({ products });
        }
        else {
            const products = await ProductModel.getProducts(primaryId, "both", siteName);
            this.setState({ products });
        }
